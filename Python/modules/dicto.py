...
class dicto(Tk):
    def __init__(self, title="Dicto"):
        ...
        self._close_tab_button = Button(text="[×] Close current tab", command=self._close_current_tab)
        self._close_tab_button.grid(sticky="w")

        self._tab_control.bind("<<NotebookTabChanged>>", lambda func: self._tab_changed())
    ...
    def _create_menu_bar(self):
        ...
        menu_file.add_command(label="Save as", command=self._saveAs_file)
        menu_file.add_separator()
        menu_file.add_command(label="Exit", command=self._quit_window)
